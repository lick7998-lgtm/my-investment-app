...
for i, (symbol, name) in enumerate(tickers.items()):
    ...
    with cols[i]:
        if res:
            ...
            st.markdown(f"<div class='value-text' style='color:{h_color};'>當前點數：{int(curr):,}</div>", unsafe_allow_html=True)
            st.markdown(f"<div class='value-text' style='color:{h_color};'>季線 MA60：{int(m60):,}</div>", unsafe_allow_html=True)
            st.markdown(f"<div class='value-text' style='color:{h_color};'>年線 MA240：{int(m240):,}</div>", unsafe_allow_html=True)
