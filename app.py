...
/* 能量條底槽：色調 30 的暗色背景 */
.energy-bar-container { background-color: #121212; border-radius: 8px; width: 100%; height: 24px; margin-top: 8px; overflow: hidden; border: 1px solid #333; }
/* 能量條填充：色光 100 -> 250 漸層 */
.energy-bar-fill { height: 24px; border-radius: 8px; transition: width 0.6s ease-in-out; }
</style>
""", unsafe_allow_html=True)

# --- 資料抓取與精確計算 (解決 ValueError) ---
def fetch_index_data(symbol):
    try:
        data = yf.download(symbol, period="2y", progress=False)
        if data.empty: return None
        # 使用 .item() 確保取得純數值，避免 Series 報錯
...
def get_style_config(pct, current, ma240):
    if current < ma240: # 跌破年線 -> 紅燈
        ...
        status, color_end = "🟢 綠燈 (季線之上)", "#00FF00"
...
for i, (symbol, display_name) in enumerate(tickers.items()):
    ...
    with (col_display1 if i == 0 else col_display2):
        if res:
            ...
            # 能量條長度：連動距季線百分比 (上限 40%)
            fill_width = min(max(abs(pct), 5.0), 40.0) # 保底 5% 讓漸層色看得見
            st.markdown(f"""
